...
#import "HLHJColumnMenuCell.h"
#import "HLHJColumnMenuHeaderView.h"
...
@interface HLHJColumnMenu ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
/** navSaveBtn */
@property (nonatomic, weak) UIButton *navSaveBtn;
/** tags */
...
/** 头部视图 */
@property (nonatomic, weak) HLHJColumnMenuHeaderView *headerView;
/** 长按手势 */
...
- (instancetype)initWithTagsArrM:(NSMutableArray *)tagsArrM OtherArrM:(NSMutableArray *)otherArrM Type:(HLHJColumnMenuType)type Delegate:(id<HLHJColumnMenuDelegate>)delegate{
    if (self = [super init]) {
        ...
        for (int i = 0; i < tagsArrM.count; i++) {
            HLHJDefaultModel *cmodel = tagsArrM[i];
            HLHJColumnMenuModel *model = [[HLHJColumnMenuModel alloc] init];
            model.title = cmodel.category_name;
            model.ID = cmodel.ID;
            model.type = type;
            if (self.type == HLHJColumnMenuTypeTouTiao) {
                model.showAdd = NO;
                model.selected = NO;
                if (i == 0) {
                    model.resident = YES;
                }
            } else if (type == HLHJColumnMenuTypeTencent) {
                if (i != 0) {
                    model.selected = YES;
                } else {
                    model.selected = NO;
        ...
        for (HLHJAllModel *aModel in otherArrM) {
            HLHJColumnMenuModel *model = [[HLHJColumnMenuModel alloc] init];
            model.title = aModel.category_name;
            model.ID  = aModel.ID;
            if (self.type == HLHJColumnMenuTypeTouTiao) {
                model.showAdd = YES;
            }
...
- (void)initColumnMenuUI {
    ...
    navView.backgroundColor = [TMEngineConfig instance].themeColor;
    ...
    navCloseBtn.frame = CGRectMake(20, CGRectGetMinY(self.navTitle.frame), 20, 20);
    NSBundle *bundle = [NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:@"HLHJImageResources" ofType:@"bundle"]];
    ...
    navSaveBtn.frame = CGRectMake(self.navView.width - 50, CGRectGetMinY(self.navTitle.frame), 40, 20);
    navSaveBtn.titleLabel.font = [UIFont systemFontOfSize:15];
    [navSaveBtn setTitle:@"保存" forState:UIControlStateNormal];
    [navSaveBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    self.navSaveBtn = navSaveBtn;
    [navSaveBtn addTarget:self action:@selector(navSaveBtnClick) forControlEvents:UIControlEventTouchUpInside];
    [self.navView addSubview:navSaveBtn];
    ...
    //注册cell
    [self.collectionView registerClass:[HLHJColumnMenuCell class] forCellWithReuseIdentifier:CELLID];
    [self.collectionView registerClass:[HLHJColumnMenuHeaderView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:HEADERID];
    
    //添加长按的手势
    self.longPress = [[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(longPress:)];
    if (self.type == HLHJColumnMenuTypeTencent) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
   HLHJColumnMenuCell  *cell = [collectionView dequeueReusableCellWithReuseIdentifier:CELLID forIndexPath:indexPath];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    {
        HLHJColumnMenuHeaderView *headerView = [self.collectionView dequeueReusableSupplementaryViewOfKind:kind withReuseIdentifier:HEADERID forIndexPath:indexPath];
        if(headerView == nil)
        {
            headerView = [[HLHJColumnMenuHeaderView alloc] init];
        }
        if (indexPath.section == 0) {
            headerView.titleStr = @"我的频道";
//            headerView.detailStr = @"按住拖动调整排序";
               headerView.detailStr = @"";
            if (self.type == HLHJColumnMenuTypeTouTiao) {
...
- (void)headViewEditBtnClick {
    ...
    if ([self.editBtnStr containsString:@"编辑"]) {
        ...
        for (int i = 0; i < self.tagsArrM.count; i++) {
            HLHJColumnMenuModel *model = self.tagsArrM[i];
            if (i == 0) {
                model.selected = NO;
            } else {
                model.selected = YES;
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    HLHJColumnMenuModel *model;
    if (indexPath.section == 0) {
        model = self.tagsArrM[indexPath.item];
        //判断是否是编辑状态
        if ([self.editBtnStr containsString:@"编辑"]) {
            //判断是否是头条,是就直接回调出去
            if (model.type == HLHJColumnMenuTypeTouTiao) { //头条
       ...
       HLHJColumnMenuCell  *cell = (HLHJColumnMenuCell *)[collectionView cellForItemAtIndexPath:indexPath];
        cell.closeBtn.hidden = YES;
        model  = self.otherArrM[indexPath.item];
        if (model.type == HLHJColumnMenuTypeTencent) {
            model.selected = YES;
        } else if (model.type == HLHJColumnMenuTypeTouTiao) {
            if ([self.editBtnStr containsString:@"编辑"]) {
                model.selected = NO;
            } else {
                model.selected = YES;
...
- (void)colseBtnClick:(UIButton *)sender {
    HLHJColumnMenuModel *model = self.tagsArrM[sender.tag];
    model.selected = NO;
...
- (void)refreshDelBtnsTag {
    for (HLHJColumnMenuCell *cell in self.collectionView.visibleCells) {
...
//    for (HLHJColumnMenuModel *model in self.tagsArrM) {
//        [tempTagsArrM addObject:model.title];
//    }
//    for (HLHJColumnMenuModel *model in self.otherArrM) {
