...
- (void)setModel:(HLHJNewsConmmetModel *)model {
    ...
    NSString *avatar = @"";
    if ([model.avatar containsString:@"://"]) {
        avatar = model.avatar;
    }else {
        avatar = [NSString stringWithFormat:@"%@%@",BASE_URL,model.avatar];
    }

    [self.iconImg sd_setImageWithURL:[NSURL URLWithString:avatar] placeholderImage:[UIImage imageNamed:@"HLHJImageResources.bundle/ic_touxiang"]];
